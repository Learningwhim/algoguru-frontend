 ...
 */
export default function Header({ step, totalSteps, onMenuToggle }) {
  const navigate = useNavigate();
  const { theme, toggleTheme } = useTheme();
  const isDark = theme === "dark";

  return (
    <header className="sticky top-0 z-50 w-full px-4 md:px-8 py-3 md:py-4 flex items-center justify-between bg-gradient-to-b from-[#EAF3FA] via-[#EAF3FA]/70 to-transparent dark:from-[#0B0B0A] dark:via-[#0B0B0A]/70 dark:to-transparent backdrop-blur-sm">
      <div className="flex items-center gap-3">
        {onMenuToggle && (
          <button 
            onClick={onMenuToggle}
            className="md:hidden cursor-pointer w-9 h-9 flex items-center justify-center rounded-md hover:bg-[#16223A]/[0.06] dark:hover:bg-white/[0.06] transition-colors"
          >
            <Menu size={20} className="text-[#16223A] dark:text-white" />
          </button>
        )}
        <span
          onClick={() => navigate("/")}
